...
public class CouponController {
    ...
    public String couponList(HttpServletRequest request, HttpSession session) {
        ...
        List<NewBeeMallCouponVO> coupons = newBeeMallUserCouponService.selectAvailableCoupon(userId);
    ...
    public String myCoupons(HttpServletRequest request, HttpSession session) {
        ...
        List<NewBeeMallCouponVO> coupons = newBeeMallUserCouponService.selectMyCoupons(userVO.getUserId());
    ...
    public Result save(@PathVariable Long couponId, HttpSession session) {
        NewBeeMallUserVO userVO = (NewBeeMallUserVO) session.getAttribute(Constants.MALL_USER_SESSION_KEY);
        return ResultGenerator.genDmlResult(newBeeMallUserCouponService.saveCouponUser(couponId, userVO.getUserId()));
    }

    @ResponseBody
    @DeleteMapping("coupon/{couponUserId}")
    public Result delete(@PathVariable Long couponUserId) {
        return ResultGenerator.genDmlResult(newBeeMallUserCouponService.deleteCouponUser(couponUserId));
    }
}
